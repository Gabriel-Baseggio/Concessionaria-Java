...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            new Cliente("Marcio", "134.431.869-15", "123").addUsuario();
    ...
    private static void menuUsuario() {
        do {
            ...
            int escolha = inputInt(usuarioLogado.menu());

            try {
                if (escolha < 0 || escolha > 17) {
                    throw new OpcaoInvalidaException();
                } else if (escolha > 3 && !(usuarioLogado instanceof Funcionario)) {
    ...
    private static void verMeusVeiculos() {
        System.out.println();
        System.out.println(usuarioLogado.verMeusVeiculos().toString());
    }
    ...
    private static void venderVeiculo() {
        if (usuarioLogado instanceof Funcionario funcionario) {
            ...
            try {
                veiculo = Veiculo.buscarVeiculo(cod);
                funcionario.venderVeiculo(Usuario.buscarUsuario(cpf), veiculo);
    ...
    private static void cadastrarUsuario() {
        if (usuarioLogado instanceof Gerente gerente) {
            ...
            try {
                switch (opcao) {
                    case 1 -> gerente.cadastrarUsuario(new Cliente(nome, cpf, senha));
                    case 2 -> {
    ...
    private static void removerUsuario() {
        if (usuarioLogado instanceof Gerente gerente) {
            ...
            try {
                ...
                gerente.removerUsuario(usuario);
    ...
    private static void editarUsuario() {
        if (usuarioLogado instanceof Gerente gerente) {
            ...
            try {
                usuarioAntigo = Usuario.buscarUsuario(cpf);
            } catch (ObjetoNaoEncontradoException exception) {
                System.out.println(exception.getMessage());
                return;
            }

            if (opcao == 1 && !(usuarioAntigo instanceof Cliente)) {
                System.out.println("Esse usuário não é um cliente!");
                return;
            }
            if (opcao == 2 && !(usuarioAntigo instanceof Vendedor)) {
            ...
            switch (opcao) {
                case 1 -> gerente.editarUsuario(usuarioAntigo, new Cliente(nome, cpf, senha));
                case 2 -> {
                    int matricula = inputInt("Digite a matrícula do vendedor: ");
                    double salario = inputDouble("Digite o salário do vendedor: ");
                    gerente.editarUsuario(usuarioAntigo, new Vendedor(nome, cpf, senha, matricula, salario));
                }
            }
